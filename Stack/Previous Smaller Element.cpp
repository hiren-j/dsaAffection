// Program to find the the previous smallest element of each element of an array ~ coded by Hiren
#include <iostream>
#include <cstdlib>
#include <vector>
#include <stack>
...
public:
    ...
    vector<ll> prevSmallestElement_V1(vector<ll>& nums, int n) {
        // Iterate the array
        for(int i=n-1; i>=0; --i) {
            // Stores the previous smaller element of the ith element
            ll prevSmallest = -1;
            // Iterate and find the previous smaller element
            for(int j=i-1; j>=0; --j) {
                if(nums[j] < nums[i]) {
                    prevSmallest = nums[j];
                    break;
            ...
            // Replace the ith element by its previous smaller element or -1 if not found
            nums[i] = prevSmallest;
        }
    ...
    // #2 Method to find the previous smallest element of each element, using buffer and 1 stack - O(N) & O(N)
    vector<ll> prevSmallestElement_V2(vector<ll>& nums, int n) {
    ...
    // #3 Method to find the previous smallest element of each element, using 1 stack - O(N) & O(N)
    vector<ll> prevSmallestElement_V3(vector<ll>& nums, int n) {
        // Stores the elements in monotonic fashion (increasing order)
        stack<ll> st;
...
int main() {
    ...
    while(userWantsOperation) {
        ...
        // Call to find the previous smallest element of each element of the array
        Solution solution;
        nums = solution.prevSmallestElement_V3(nums, n);
