...
// Class to implement the first solution :-
class Solution_V1 {
...
public:
    ...
    class TopDown {
        ...
        int solveWithMemo(vector<int>& memory, string& s, int n, int startIndex) {
            ...
            // Perform all the palindrome partitions on the index and update the result by the minimum value
            for(int index = startIndex; index < n; ++index) {
                if(isPalindrome(s, startIndex, index)) {
                    int currCuts = solveWithMemo(memory, s, n, index + 1);
                    minCuts = (index != n-1 ? minCuts = min(minCuts, 1 + currCuts) : currCuts);
                }
            }
        ...
        int solveWithMemo(vector<vector<int>>& memory1, vector<int>& memory2, const string& s, int n, int startIndex) {
            ...
            for(int index = startIndex; index < n; ++index) {
                if(isPalindrome(memory1, s, startIndex, index)) {
                    int currCuts = solveWithMemo(memory1, memory2, s, n, index + 1);
                    minCuts = (index != n-1 ? minCuts = min(minCuts, 1 + currCuts) : currCuts);
                }
            }
