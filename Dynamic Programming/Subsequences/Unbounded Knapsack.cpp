// Program to find the maximum total value you can collect for the knapsack (each item can be taken any number of times) ~ coded by Hiren
#include <iostream>
#include <vector>
using namespace std;
...
private:
    ...
    int solveWithMemo(vector<vector<int>>& memory, vector<int>& weights, vector<int>& values, int N, int index, int capacity) {
        ...
        int currSkip = solveWithMemo(memory, weights, values, N, index + 1, capacity); // Is to skip the index value and move to the next index 
    ...
    int solveWithoutMemo(vector<int>& weights, vector<int>& values, int N, int index, int capacity) {
        ...
        // There are always two possibilities to perform at each index
        int currSkip = solveWithoutMemo(weights, values, N, index + 1, capacity); // Is to skip the index value and move to the next index
        int currTake = 0;                                                         // Is to take the index value and stay at the same index
...
public:
    ...
    int unboundedKnapsack_A(int N, int capacity, vector<int>& values, vector<int>& weights) {
        ...
        for(int index = 1; index <= N; ++index) {
            for(int currCapacity = 0; currCapacity <= capacity; ++currCapacity) {
                ...
                if(currCapacity - weights[index - 1] >= 0) {
                    currTake = values[index - 1] + dp[index][currCapacity - weights[index - 1]];
                }
    ...
    int unboundedKnapsack_B(int N, int capacity, vector<int>& values, vector<int>& weights) {
        ...
        for(int index = 1; index <= N; ++index) {
            for(int currCapacity = 0; currCapacity <= capacity; ++currCapacity) {
                ...
                int currTake = 0;
                if(currCapacity - weights[index - 1] >= 0) {
                    currTake = values[index - 1] + currRow[currCapacity - weights[index - 1]];
                }
...
int main() {
    ...
    while(userRunsApp) {
        ...
        if(N <= 0 || N >= 1001 || capacity <= 0 || capacity >= 1001) {
            cout<<"You must enter the size and the capacity which lies between 1 and 10^3!";
        }
        else {
            ...
            for(int index = 0; index < N;) {
                ...
                // Check the given values are lying within the problem constraints or not
                if(values[index] <= 0 || values[index] >= 100000001 || weights[index] <= 0 || weights[index] >= 100000001) {
                    cout<<"You must enter the value and the weight which lies between 1 and 10^8!\n";
                    continue;
                }
                index++;
            }
            ...
            BottomUp bottomUp;
            int maxValue = bottomUp.unboundedKnapsack_B(N, capacity, values, weights);
