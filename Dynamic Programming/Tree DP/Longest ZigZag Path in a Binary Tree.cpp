    ...
    DON'T IGNORE MUST READ (REGARDING TIME AND AUXILIARY SPACE OF THE FUNCTION):
        ...
        `getLongestPath() without memoization` 
            - Time complexity: O(N*N) 
              Explanation: The total number of nodes is N. The auxiliary time of the `getLongestPath()` is O(N) here and as we're calling the function from each node of the tree hence the overall time complexity becomes O(N*N).
        ...
        `getLongestPath() with memoization` 
            - Time complexity: O(NLogN) 
              ...
              More clearly Log(N) time, the results will be calculated once after that the memoization table will be used to return the result in Log(N) time, 
              as we're calling the `getLongestPath()` for each node then the overall time complexity becomes O(NLogN). 
                          
            - Auxiliary space : O(N*2 + H)
              Explanation: The first term N*2 represents the space taken by the memoization table. It will end up storing results for N*2 states in the worst case.
