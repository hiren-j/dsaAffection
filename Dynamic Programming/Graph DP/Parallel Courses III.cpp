...
public:
    // Method to find minimum number of months needed to complete all the courses, using recursion with memoization - O(V+E) & O(V+E)
    int minimumTime(int n, vector<vector<int>>& relations, vector<int>& time) {
        ...
        for(auto& r : relations) {
            ...
            int totalNeighbors = adjList[prevCourse].size();
            if(totalNeighbors > maxNeighbors) {
                source = prevCourse;
                maxNeighbors = totalNeighbors;
            }
        }
