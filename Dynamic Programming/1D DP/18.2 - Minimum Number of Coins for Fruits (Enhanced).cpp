...
class TopDown {
    ...
    int solveWithoutMemo(vector<int>& prices, int index) {
        ...
        int minCoins  = solveWithoutMemo(prices, index + jumpLimit + 1); // We know the total free fruits, hence consider skiping all of them at once
        
        for(int jump = 1; (jump <= jumpLimit && index + jump <= n); ++jump) {
            int nextBuy = solveWithoutMemo(prices, index + jump); // Now consider purchasing the next fruit, hence move to that start point
    ...
    int solveWithMemo(vector<int>& dp, vector<int>& prices, int index) {
        ...
        int jumpLimit = index + 1;
        int minCoins  = solveWithMemo(dp, prices, index + jumpLimit + 1); // We know the total free fruits, hence consider skiping all of them at once

        for(int jump = 1; (jump <= jumpLimit && index + jump <= n); ++jump) {
            int nextBuy = solveWithMemo(dp, prices, index + jump); // Now consider purchasing the next fruit, hence move to that start point
            minCoins = min(minCoins, nextBuy);
        }
