...
class TopDown {
    ...
    int solveWithoutMemo(vector<int>& prices, int index) {
        ...
        for(int jump = 1; (jump <= jumpLimit && index + jump <= n); ++jump) {
            ...
            int nextFree  = solveWithoutMemo(prices, index + jump + 1); // Consider the next fruit for free, hence skip that start point
    ...
    int solveWithMemo(vector<int>& dp, vector<int>& prices, int index) {
        ...
        for(int jump = 1; (jump <= jumpLimit && index + jump <= n); ++jump) {
            ...
            int nextFree  = solveWithMemo(dp, prices, index + jump + 1); // Consider the next fruit for free, hence skip that start point
            int currCoins = prices[index] + min(nextBuy, nextFree);
            minCoins = min(minCoins, currCoins);
        }
...
public:
    ...
    int minimumCoins(vector<int>& prices) {
        ...
        vector<int> dp(n + 2, -1);
        dp[n] = dp[n + 1] = 0; // Init the edge case
