...
int main() {
    ...
    if(testCases <= 0) {
        cout<<"Enter a valid number for the testcases, application expects a positive integer!";
        return 0;
    }
        
    while(testCases--) {
        string s; 
        cout<<"\nEnter a sentence: "; getline(cin, s);
        Solution solution;
        string output = solution.reverseWords_V2(s);        
        cout<<"Sentence after words reversal: "<<output<<'\n';
    }
